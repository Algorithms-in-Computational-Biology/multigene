...
void GAlign::InitStrings(char *s1, char *s2, int s1len, int s2len) {
	seq1 = s1;
	seq1len = s1len;
	seq2 = s2;
	seq2len = s2len;
	...
	for (j = startRow; j <= seq2len; j++) {
		...
		for (i = 1; i <= seq1len; i++) {
			...
			if (enthalpy1 < forbidden_enthalpy) {
				entropy1 += GNNParams->GetEntropy(prevchari, seq1char,
						prevcharj, seq2char);
				enthalpy1 += GNNParams->GetEnthalpy(prevchari, seq1char,
						prevcharj, seq2char);
			}
			if (enthalpy2 < forbidden_enthalpy) {
				enthalpy2 += GNNParams->GetEnthalpy(prevchari, seq1char, 0,
						seq2char); // 0 is gap!
				entropy2 += GNNParams->GetEntropy(prevchari, seq1char, 0,
						seq2char);
			}
			if (enthalpy3 < forbidden_enthalpy) {
				enthalpy3 += GNNParams->GetEnthalpy(0, seq1char, prevcharj,
						seq2char);
				entropy3 += GNNParams->GetEntropy(0, seq1char, prevcharj,
						seq2char);
			}
			...
			if (i == 1) {
				...
				if (enthalpy1 < forbidden_enthalpy) {
					entropy1 += GNNParams->GetEntropy(seq1char, 0, prevcharj,
							seq2char);
					enthalpy1 += GNNParams->GetEnthalpy(seq1char, 0, prevcharj,
							seq2char);
				}
				if (enthalpy2 < forbidden_enthalpy) {
					entropy2 += GNNParams->GetEntropy(0, 0, prevcharj,
							seq2char);
					enthalpy2 += GNNParams->GetEnthalpy(0, 0, prevcharj,
							seq2char);
				}
			}
			// -- choose optimum combination
...
bool GAlign::OutputAlignment(ostream &outputStream, int i, int j, int t,
		bool local) {
...
//
//  RETURN VALUE:
//      float  - Entropy (dS) in e.u (cal/K.mol)
//
//  REVISION HISTORY
//  $              00sep23 : created LK
//  #$
...
//
//  RETURN VALUE:
//      float  - free Energy value (dG) in cal/mol at given temperature
//
//  REVISION HISTORY
//  $              00sep06 : created LK
//  #$
...
//
//  RETURN VALUE:
//      float  - free Energy value (dG) in cal/mol at melting temperature
//
//  REVISION HISTORY
//  $              00sep06 : created LK
//  #$
