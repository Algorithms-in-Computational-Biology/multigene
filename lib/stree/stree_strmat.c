                            ...
                            int M, int strid)
{
  ...
  if (tree->nextslot == tree->strsize) {
    if (tree->strsize == 0) {
      tree->strsize = 128;
      if ((tree->strings = (char **)malloc(tree->strsize * sizeof(char *))) == NULL ||
          (tree->rawstrings = (char **)malloc(tree->strsize * sizeof(char *))) == NULL)
        return -1;
      ...
      if ((tree->strings = (char **)realloc(tree->strings,
                                   tree->strsize * sizeof(char *))) == NULL ||
          (tree->rawstrings = (char **)realloc(tree->rawstrings,
                                      tree->strsize * sizeof(char *))) == NULL)
        return -1;
      if ((tree->lengths = (int *)realloc(tree->lengths,
                                   tree->strsize * sizeof(int))) == NULL ||
          (tree->ids = (int *)realloc(tree->ids,
                               tree->strsize * sizeof(int))) == NULL)
        return -1;
  ...
  case LIST_THEN_ARRAY:
    if (!parent->isanarray) {
      ...
      if (count + 1 < tree->build_threshold) {
        ...
        children = (stree_node **)malloc(tree->alpha_size * sizeof(STREE_NODE));
        if (children == NULL)
          return NULL;
...
STREE_INTLEAF int_stree_new_intleaf(SUFFIX_TREE tree, int strid, int pos)
{
  ...
  if ((ileaf = (stree_intleaf *)malloc(sizeof(SINTLEAF_STRUCT))) == NULL)
    return NULL;
                              ...
                              int leafpos)
{
  ...
  if ((leaf = (stree_leaf*)malloc(sizeof(SLEAF_STRUCT))) == NULL)
    return NULL;
                              ...
                              char *rawedgestr, int edgelen)
{
  ...
  if (tree->build_type == COMPLETE_ARRAY) {
    node->children = (stree_node *)malloc(tree->alpha_size * sizeof(STREE_NODE));
