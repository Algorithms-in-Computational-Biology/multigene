...
{
  if ((stack = (lvlist **)malloc(num_nodes * sizeof(LEFTVALS))) == NULL)
    return 0;
...
int lvals_add_value(int id, int value, int amount)
{
  ...
  if ((node = (lvlist *)malloc(sizeof(LVNODE))) == NULL)
    return 0;
...
//this one takes already built tree
...
//It seems min_length is not being used
//I added it myself
//Also, the code does not find maximal matches (they have to be
//at lest near-supermaximal), I changed that by commenting 
//a part of the code
static SUPERMAXIMALS compute_supermax(SUFFIX_TREE tree, STREE_NODE node,
                                      int min_percent, int min_length,
                                      SUPERMAXIMALS list)
{
  ...
  if (((min_percent == 0) || 
      (min_percent < 100 && percent >= min_percent) ||
      (min_percent == 100 && witnesses == num_leaves)) &&
      (stree_get_labellen(tree,node)>=min_length)) {

    if ((newnode = (supermax_node *)malloc(sizeof(STRUCT_SUPERMAX))) == NULL) {
      errorflag = 1;
